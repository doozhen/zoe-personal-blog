...
app.get('/api/avatar', async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM avatar ORDER BY created_at DESC LIMIT 1');
        if (result.rows.length > 0) {
...
app.post('/api/avatar', upload.single('avatar'), async (req, res) => {
    try {
        ...
        const fileUrl = await uploadToCloudinary(req.file, 'zoe-blog/avatar');

        // Delete existing avatar if any
        await pool.query('DELETE FROM avatar');

        // Insert new avatar
        await pool.query('INSERT INTO avatar (url) VALUES ($1)', [fileUrl]);
...
app.delete('/api/avatar', async (req, res) => {
    try {
        ...
        if (result.rows.length > 0) {
            ...
            const publicId = avatar.url.substring(avatar.url.lastIndexOf('/') + 1, avatar.url.lastIndexOf('.'));
            await cloudinary.uploader.destroy(`zoe-blog/avatar/${publicId}`);
            await pool.query('DELETE FROM avatar');
        }
